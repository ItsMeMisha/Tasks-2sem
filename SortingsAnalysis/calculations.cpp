...
SortingsAnalyzerClass<int>* generateArray (std::size_t size) {
    ...
    SortingsAnalyzerClass<int>* array = new SoringsAnalyzerClass<int>[size];
    for (std::size_t i = 0; i < size; ++i)
        array[i] = rand() % 1024 + 8;
}

Results CountActionsDuringSorting (std::size_t size, const Sorting& sort) {

    SortingsAnalyzerClass<int>* array = generateArray (size);
    sort(array, size);
    delete[] array;

    Results results = {size, SortingsAnalyzerClass<int>::numOfComparisons, SortingsAnalyzerClass<int>::numOfAssignments};
